pacman::p_load(tidyverse,vroom,gganimate, av, gifski, ggplot2)
...
a <- t_data %>% 
  ...
  geom_segment(data = gridlines, aes(x=x, y=y, xend=xend, yend=yend),
               color=c("yellow","green","yellow"), inherit.aes = FALSE) +
...
animate(a, renderer = gifski_renderer(), width=4.155, height=4.5, unit="in", res=300,)

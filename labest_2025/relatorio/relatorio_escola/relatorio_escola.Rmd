...
# Introdução
Este relatório apresenta indicadores escolares referentes ao período de 2023-2024, incluindo número de escolas, educandos, atividade e razões derivadas. Analisamos por região e UF, comparando cada unidade à média nacional.
...
ggplot(df_regiao_razao_atividade, aes(
  ...
  annotate("text",
           x = global_razao_atividade_por_escola,
           y = 0.5,
           label = paste0("          ", round(global_razao_atividade_por_escola,2)),
           angle = 0,
           vjust = 0.5) +
  geom_text(aes(label = round(razao_atividade_por_escola,2)), hjust = -0.1) +
  labs(title = "Razão atividade por Escola por Região", x = "Razão", y = "Região") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2")
```
```{r tabela-regiao-atividade, echo=TRUE}
...
ggplot(df_regiao_razao_educandos, aes(
  ...
  annotate("text",
           x = global_razao_educando_atividade,
           y = 0.5,
           label = paste0("           ", round(global_razao_educando_atividade,2)),
           angle = 0,
           vjust = 0.5) +
  geom_text(aes(label = round(razao_educando_atividade,2)), hjust = -0.1) +
  labs(title = "Razão Educandos por atividade por Região", x = "Razão", y = "Região") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2")
```
...
## Razões por UF
### atividade por Escola
```{r uf-atividade, fig.width=10, echo=TRUE}
...
ggplot(df_uf_razao_atividade, aes(
  ...
  annotate("text",
           ...
           vjust = -0.5) +
  geom_text(aes(label = round(razao_atividade_por_escola,2)), hjust = -0.1) +
  labs(title = "Razão atividade por Escola por UF", x = "Razão", y = "UF") +
  theme_minimal()
```
```{r tabela-uf-atividade, echo=TRUE}
...
ggplot(df_uf_razao_educandos, aes(
  ...
  annotate("text",
           ...
           vjust = -0.5) +
  geom_text(aes(label = round(razao_educando_atividade,2)), hjust = -0.1) +
  labs(title = "Razão Educandos por atividade por UF", x = "Razão", y = "UF") +
  theme_minimal()
```
